...
const MuscleConcern = () => {
  ...
  useEffect(() => {
    if (!user) return;
    const load = async () => {
      const { data } = await supabase.from("onboarding_data").select("muscle_concern").eq("user_id", user.id).single();
  ...
  const handleNext = async () => {
    ...
    setSaving(true);
    try {
      await supabase.from("onboarding_data").update({ muscle_concern: selected }).eq("user_id", user.id);
    ...
    >
      <section className="mb-10">
        <h1 className="font-headline font-bold text-4xl md:text-5xl text-on-surface leading-tight tracking-tight mb-4">
          How concerned are you about muscle loss?
        </h1>
        <p className="text-on-surface-variant text-lg max-w-md">
          Be honest — this helps us calibrate the intensity of your protocol.
        </p>
      </section>
        ...
        {concerns.map((c, i) => (
          ...
          >
            <div className={`w-14 h-14 rounded-full flex items-center justify-center shrink-0 ${
              selected === c.value ? "bg-gradient-to-br from-primary to-primary-container" : "bg-surface-container-highest"
            }`}>
              <span className={`material-symbols-outlined text-2xl ${
                selected === c.value ? "text-on-primary" : c.color
              }`}>{c.icon}</span>
            </div>
